...
class Pokémon:
    ...
    """
    def __init__(self, name, atk, dif, spd, spatk,
                  spdif, hp, typing, moves = ['placeholder']):
        self.name = name
        self.atk = atk
        self.dif = dif
        self.spd = spd
    ...
    def original_stats(self):
        """Create a copy of the Pokémon's stats to use as a reference for moves.
        
        These copies will help to code and apply the effects of certain moves
        correctly, like Life Dew or Belly Drum. Life Dew will not restore hp
        beyond the user's full hp, and Belly Drum will always damage the user by
        half of its original hp. This functions is meant to be used before the
        Pokémon battle starts to garantee its usability.
        """
        self.og_hp = self.hp
        self.og_atk = self.atk
        self.og_dif = self.dif
        self.og_spd = self.spd
        self.og_spatk = self.spatk
        self.og_spdif = self.spdif
...
life_dew = move('life dew', 0, 100, 100, 'water', 0, 'status', 'heal', 0.5)
...
shadow_ball = move('shadow ball', 80, 20, 100, 'ghost', 417, 'special', 'opp_spdef', 0.66)
